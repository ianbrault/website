...
function getRandomWindowLocation(popup_w, popup_h) {
    const x_range = w - popup_w;
    const y_range = h - popup_h;
...
function dragWindow(event, win) {
    ...
    const win_x = cursor_x - event.clientX;
    const win_y = cursor_y - event.clientY;
...
function dragMouseDown(event) {
    const target_fields = event.target.id.split("-");
    const win_num = target_fields[target_fields.length - 1];
    const win = document.getElementById(`nt-window-${win_num}`);
...
function GeneBlockContent() {
    ...
    const geneBlock = document.createElement("img");
...
function NTWindow(width, height, centered) {
    ...
    if (centered) {
        ...
        const [x, y] = getRandomWindowLocation(width, height);
        win.style.left = `${x}px`;
        win.style.top  = `${y}px`;
    }
...
function createPopup() {
    ...
    const popup = NTWindow(POPUP_WIDTH[type], POPUP_HEIGHT[type]);
...
const paintMenu = NTMenuBar(paintIcon, function(){});
paintMenu.style.backgroundColor = "rgb(3, 0, 111)";
paintWindow.appendChild(paintMenu);

const paintBg = document.createElement("img");
...
const starburst = document.createElement("img");
...
function openTicketsInfo() {
    ...
    const width = Math.min(500, Math.floor(w * 0.95));
    const container = NTWindow(width, "auto", true);
    container.classList.add("tickets-info");
    const win_num = n_windows - 1;
...
const releaseDate = new Date("September 12, 2018 10:00:00");
function updateCountdown() {
    ...
    const offset_s = Math.floor(offset_ms / 1000);
    const sec = format(offset_s % 60);
    const min = format(((offset_s - sec) / 60) % 60);
    const hrs = format((offset_s - (min*60) - sec) / 3600);
